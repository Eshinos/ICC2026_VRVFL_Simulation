...
MINIMUM_RATE_SUCCESS_CHANCE_THRESHOLD=0.50
def VRVFL(accuracy_list, loss_list, alpha, save_dir_name = None, num_local_steps = NUM_LOCAL_STEPS, verbose = True, num_comm_rounds = NUM_COMMUNICATION_ROUNDS):
    VRVFLtime = 0
    roundtimes= []
    ...
    for round in range(num_comm_rounds):
        ...
        for vehicle in feasibleVehicles:
            ...
            secondTermConstantVector.append( -(1 - vehicle.epsilon**2)/(vehicle.h_hat_squared * vehicle.epsilon**2)) # h_hat_squared * epsilon^2 / (1-epsilon^2) for each vehicle
        ...
        results = bcd_alg(len(feasibleVehicles), NUM_RESOURCE_BLOCKS, alpha,Dv_overD_vector, bcdLambda_vector, 1.0, DEFAULT_EPSILON, y_max_vec, y_min_vec, 50, 1e-3, None, None, cp.SCS, False, DEFAULT_EPSILON,secondTermConstantVector)
        ...
        for idx, vehicle in enumerate(feasibleVehicles):
            vehicle.u_vt = u_t[idx]  # Assign the solved u value
            if verbose:
                print(f"u_vt: {vehicle.u_vt}")
            vehicle.yt = y_vec[idx]  # Assign the solved y value

        selected_indices=weighted_sampling_without_replacement(u_t, NUM_RESOURCE_BLOCKS)
