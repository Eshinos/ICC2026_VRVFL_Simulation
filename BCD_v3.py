...
def term2_y_step(u_val, y_var, constant_vec, N, alpha_val):
    """ Term 2 (new): (1-alpha) * max_n( u_n * constant_n * log y_n ) """
    if alpha_val == 1:
        return 0
    terms = [u_val[n] * constant_vec[n] * cp.log(y_var[n]) for n in range(N)]
    return (1 - alpha_val) * cp.max(cp.hstack(terms))
...
def solve_bcd_optimization_v2(
    ...
    secondTermConstantVector: np.ndarray | None = None
) -> dict:
    """
    Solves the optimization problem using Block Coordinate Descent (BCD) - Version 2 Script Logic.

    Minimizes obj based on the structure in the second provided script.

    Args:
        N: Number of elements.
        K: Sum constraint for u (sum(u) <= K).
        alpha: Weighting parameter in the objective.
        D: Data vector D_n (shape N).
        lambda_vec: Data vector lambda_n (shape N).
        y_th: Threshold parameter in the objective.
        u_min: Minimum value for u_n.
        y_max_vec: Initial upper bounds for y_n (shape N). Adjusted internally if y_min > y_max.
        y_min_vec: Initial lower bounds for y_n (shape N). Adjusted internally if y_min > y_max.
        max_iterations: Maximum number of BCD iterations.
        tolerance: Convergence tolerance for relative objective change.
        initial_u: Optional initial guess for u (shape N).
        initial_y: Optional initial guess for y (shape N).
        solver: CVXPY solver to use (e.g., cp.SCS, cp.ECOS).
        verbose: If True, prints progress during iterations.
        epsilon: Small constant for numerical stability.

    Returns:
        A dictionary containing:
        - 'status': 'converged', 'max_iterations_reached', 'failed', 'infeasible_init'.
        - 'u': Final optimized u vector (or last valid u if failed).
        - 'y': Final optimized y vector (or last valid y if failed).
        - 'objective': Final objective value (or None if failed early).
        - 'iterations': Number of iterations performed.
        - 'objective_history': List of objective values per iteration (from u-step).
    """
    if verbose:
    ...
    #          u_current = u_current * (K / u_sum) * 0.9999
    #          u_current = np.maximum(u_current, u_min) # Ensure u_min holds
    # else:
    #     assert initial_u.shape == (N,), "initial_u must have shape (N,)"
    #     u_current = np.clip(initial_u, u_min, 1.0)
    #     u_sum = np.sum(u_current)
    #     if u_sum > K + epsilon:
    #          warnings.warn(f"Initial u sum ({u_sum:.4f}) > K ({K}). Clipping and possibly scaling.")
    #          u_current = u_current / u_sum * K # Ensure feasibility
    #          u_current = np.clip(u_current, u_min, 1.0) # Re-clip
    if initial_u is None:
        u_current = np.ones(N)
        u_sum = np.sum(u_current)
        if u_sum > K:
    ...
    #      y_current = np.random.uniform(y_effective_lower_bound, y_effective_upper_bound)
    ...
    for i in range(max_iterations):
        ...
        try:
            ...
            obj_y = term1(u_fixed.value, y_var, alpha_param.value, D, lambda_vec, N, epsilon) + term2_y_step(u_fixed.value, y_var, secondTermConstantVector, N, alpha_param.value)
        ...
        try:
            ...
            log_term = secondTermConstantVector * np.log(safe_y_fixed_val)
            term2_const = (1 - alpha) * log_term
            obj_u = cp.sum(cp.multiply(term1_const, cp.inv_pos(u_var))) + cp.max(cp.multiply(u_var, term2_const))
